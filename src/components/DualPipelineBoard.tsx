...
import { Opportunity, OpportunityStage, PROCESSING_PIPELINE_STAGES, GATEWAY_ONLY_PIPELINE_STAGES, STAGE_CONFIG, getServiceType, migrateStage } from "@/types/opportunity";
    ...
    "bg-zinc-800 border-r border-white/20", isCompact ? "w-7 sm:w-9 lg:w-11" : "w-9 sm:w-11 lg:w-14", "mobile-landscape:w-6", colorAccent)}>
      ...
      <div className="flex-1 flex flex-col min-h-0 min-w-0 overflow-hidden relative" data-pipeline={pipelineType}>
        ...
        msOverflowStyle: 'none'
          ...
          <div className={cn("flex min-w-max pb-0", isCompact ? "gap-0.5 p-0.5" : "gap-1 p-1", "mobile-landscape:gap-1 mobile-landscape:p-0.5")}>
            {stages.map(stage => {
        ...
        <div ref={scrollContainerRef} className={cn("flex-1 overflow-x-auto overflow-y-hidden min-h-0 relative z-10")} style={{
        WebkitOverflowScrolling: 'touch',
        touchAction: 'pan-x pan-y'
      }}>
          <div className={cn("flex items-stretch min-w-max min-h-0 pt-1", isCompact ? "gap-0.5 p-0.5" : "gap-1 p-1", "mobile-landscape:gap-1 mobile-landscape:p-0.5")}>
            {stages.map(stage => <PipelineColumn key={stage} stage={stage} opportunities={getOpportunitiesByStage(stage)} onDragStart={onDragStart} onDragOver={onDragOver} onDrop={(e, s) => onDrop(e, s, pipelineType)} onCardClick={onCardClick} onAssignmentChange={onAssignmentChange} onSlaStatusChange={onSlaStatusChange} onAddNew={stage === 'application_started' ? onAddNew : undefined} hideHeader={true} isCompact={isCompact} onTouchDragStart={onTouchDragStart} onTouchDragMove={onTouchDragMove} onTouchDragEnd={onTouchDragEnd} />)}
          </div>
        </div>

        {/* Mobile Navigation Indicators - hidden in mobile landscape to save vertical space */}
        {isMobile && <div className="flex-shrink-0 flex items-center justify-center gap-2 py-1.5 bg-muted/30 mobile-landscape:hidden">
            <Button variant="ghost" size="icon" className="h-6 w-6 text-muted-foreground" onClick={() => scrollToColumn(currentColumnIndex - 1)} disabled={currentColumnIndex === 0}>
              <ChevronLeft className="h-4 w-4" />
            </Button>
            
            <div className="flex items-center gap-1">
              {stages.map((_, index) => <button key={index} onClick={() => scrollToColumn(index)} className={cn("w-1.5 h-1.5 rounded-full transition-all duration-200", index === currentColumnIndex ? "bg-primary w-3" : "bg-muted-foreground/40 hover:bg-muted-foreground/60")} aria-label={`Go to column ${index + 1}`} />)}
            </div>
            
            <Button variant="ghost" size="icon" className="h-6 w-6 text-muted-foreground" onClick={() => scrollToColumn(currentColumnIndex + 1)} disabled={currentColumnIndex === stages.length - 1}>
...
const DualPipelineBoard = ({
  ...
  const {
    ...
    return {
      processingOpportunities: processing,
      gatewayOpportunities: gateway
    };
  }, [opportunities]);
  ...
  const hasGatewayOpportunity = selectedOpportunity ? opportunities.some(opp => opp.account_id === selectedOpportunity.account_id && getServiceType(opp) === 'gateway_only') : false;
  return <>
      ...
      <div className={cn("flex-1 min-h-0 w-full flex flex-col gap-2 overflow-auto orientation-stable", "mobile-landscape:flex-row mobile-landscape:overflow-hidden", isCompact ? "p-1 gap-1 mobile-landscape:p-1 mobile-landscape:gap-1" : "p-2 mobile-landscape:p-1")}>
        {/* NMI Payments Pipeline Section */}
        <div className="flex-1 min-w-0 min-h-0 flex flex-col">
          <PipelineSection title="NMI Payments Pipeline" icon={<CreditCard className={cn("text-white", isCompact ? "h-4 w-4" : "h-5 w-5")} />} stages={PROCESSING_PIPELINE_STAGES} opportunities={processingOpportunities} onDragStart={handleDragStart} onDragOver={handleDragOver} onDrop={handleDrop} onCardClick={setSelectedOpportunity} onAssignmentChange={onAssignmentChange} onSlaStatusChange={onSlaStatusChange} onAddNew={onAddNew} colorAccent="bg-primary" pipelineType="processing" isCompact={isCompact} onTouchDragStart={handleTouchStart} onTouchDragMove={handleTouchMove} onTouchDragEnd={handleTouchEnd} />
        </div>

        {/* NMI Gateway Pipeline Section */}
        <div className="flex-1 min-w-0 min-h-0 flex flex-col">
          <PipelineSection title="NMI Gateway Pipeline" icon={<Zap className={cn("text-white", isCompact ? "h-4 w-4" : "h-5 w-5")} />} stages={GATEWAY_ONLY_PIPELINE_STAGES} opportunities={gatewayOpportunities} onDragStart={handleDragStart} onDragOver={handleDragOver} onDrop={handleDrop} onCardClick={setSelectedOpportunity} onAssignmentChange={onAssignmentChange} onSlaStatusChange={onSlaStatusChange} onAddNew={onAddNew} colorAccent="bg-teal" pipelineType="gateway" isCompact={isCompact} onTouchDragStart={handleTouchStart} onTouchDragMove={handleTouchMove} onTouchDragEnd={handleTouchEnd} />
        </div>
      </div>

      <OpportunityDetailModal opportunity={selectedOpportunity} onClose={() => setSelectedOpportunity(null)} onUpdate={updates => {
      if (selectedOpportunity) {
        onUpdateOpportunity(selectedOpportunity.id, updates);
      }
    }} onMarkAsDead={onMarkAsDead} onDelete={onDelete} onConvertToGateway={onConvertToGateway} onMoveToProcessing={onMoveToProcessing} hasGatewayOpportunity={hasGatewayOpportunity} />
    </>;
};
